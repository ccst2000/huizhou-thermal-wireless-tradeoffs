# -*- coding: utf-8 -*-
"""V3-R3 s42 v4: Fig1 研究区图（R3 修订）
  - 村点编号防重叠：碰撞检测 + 径向位移 + 引线
  - 补 Esri 影像来源与获取日期注记、EPSG:3857 投影说明
  - 位置插图加边框底色、标签内置
...
def mosaic(lon0, lat0, lon1, lat1, z):
    x0f, y0f = lonlat2tile(lon0, lat1, z)
    x1f, y1f = lonlat2tile(lon1, lat0, z)
...
# ---------------- 村点 + 防重叠编号 ----------------
CHIP = 0.008 * XR       # 编号圆片半径（数据单位）
pts = []
for _, r in v.iterrows():
    x, y = ll2wm(r.lon, r.lat)
    pts.append((r.idx, x, y))
    ax.scatter(x, y, s=14, marker="o", facecolors="white", edgecolors="0.2",
               linewidths=0.5, zorder=4)

placed = []
for idx, x, y in pts:
    def free(cx, cy):
        return all((cx - qx) ** 2 + (cy - qy) ** 2 >= (2.1 * CHIP) ** 2 for qx, qy in placed)
    lx, ly = x, y
    if not free(lx, ly):
        found = False
        for rad in (2.4 * CHIP, 3.4 * CHIP, 4.6 * CHIP, 6.0 * CHIP, 7.6 * CHIP):
            for ang in np.linspace(0, 2 * np.pi, 16, endpoint=False):
                cx, cy = x + rad * np.cos(ang), y + rad * np.sin(ang)
                if free(cx, cy):
                    lx, ly = cx, cy
                    found = True
                    break
            if found:
                break
        ax.plot([x, lx], [y, ly], color="white", lw=0.7, zorder=4, alpha=0.85)
    placed.append((lx, ly))
    ax.add_patch(Circle((lx, ly), CHIP, facecolor="#ff4d4f", edgecolor="white",
                        lw=0.8, zorder=5))
    ax.text(lx, ly, str(idx), ha="center", va="center", fontsize=6.0,
...
bx0 = ext[0] + 0.04 * XR
by0 = ext[2] + 0.05 * YR
ax.plot([bx0, bx0 + bar_m], [by0, by0], color="white", lw=4.5, zorder=6, solid_capstyle="butt")
ax.plot([bx0, bx0 + bar_m], [by0, by0], color="k", lw=2.2, zorder=7, solid_capstyle="butt")
ax.text(bx0 + bar_m / 2, by0 + 0.018 * YR, "20 km", ha="center",
...
nx = ext[1] - 0.055 * XR
ny = ext[3] - 0.10 * YR
ax.annotate("", xy=(nx, ny + 0.055 * YR), xytext=(nx, ny),
            arrowprops=dict(arrowstyle="-|>", color="white", lw=2.2,
                            mutation_scale=16), zorder=7)
ax.text(nx, ny + 0.068 * YR, "N", ha="center", fontsize=10,
        color="white", fontweight="bold", zorder=7)

# 来源与投影注记（右下）
ax.text(0.995, 0.012,
        "Imagery: Esri World Imagery (Maxar, Earthstar Geographics, CNES/Airbus DS), accessed 2026-08-20 · EPSG:3857",
        transform=ax.transAxes, ha="right", va="bottom", fontsize=6.2, color="white",
        zorder=7, path_effects=[pe.withStroke(linewidth=2, foreground="black")])
...
axin.set_title("Location in China", fontsize=7.5, color="white", pad=2,

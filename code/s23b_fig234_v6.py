# -*- coding: utf-8 -*-
"""V3-R3 s23b v6: Fig2/3/4（v6 主表形态列）
R3 修订:
...
base_map(ax, "(a) Terrain-horizon openness (tSVF)")
sc = ax.scatter(mx, my, c=m.tsvf, s=14 + 4.0 * m.built_dom_ha,
                cmap="viridis", vmin=0.70, vmax=1.0,
...
sc = ax.scatter(mx, my, c=m.forest_ring_pct, s=14 + 4.0 * m.built_dom_ha,
                cmap="YlGn", vmin=15, vmax=100, edgecolors="white", linewidths=0.7, zorder=5)
fig.colorbar(sc, ax=ax, label="%", orientation="horizontal", shrink=0.62, pad=0.03)
size_legend(ax)

ax = fig.add_subplot(gs[2])
vars_ = [("built_dom_ha", "Built-up (domain)"), ("cover_dom_pct", "Coverage ratio"),
         ("elong_fd", "Elongation"), ("compact_fd", "Compactness"),
         ("edge_den_m_ha", "Edge density"), ("lps_pct", "Largest-patch share"),
         ("relief_m", "Relief"), ("tsvf", "tSVF"),
         ("forest_ring_pct", "Forest ring"), ("water_min_m", "Water dist. (min)")]
Z = []
for c, _ in vars_:
    v = m[c].astype(float)

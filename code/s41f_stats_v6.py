# -*- coding: utf-8 -*-
"""V6 s41f: 空间稳健统计链（回应 R5 P0-5/P0-6 等）
相对 s41e 的实质修改:
  1) 数据换 v6 主表（s10b 地形 + s62b 肌理，canonical 单一坐标）
  2) 【删除循环移位"空间置换"】(R5-P0-5：其 p 依赖行序，非合法置换)。
     改为【标准标签洗牌置换】(non-spatial baseline, B=9999, seed=23)，
     并在所有表/文中明确标注为 non-spatial label permutation；
     偏相关层改用【Freedman-Lane 秩残差置换】(B=9999)。
     空间不确定性的主层 = 区组 bootstrap 95% CI（B=2999, seed=11）；
     ESS/Moran 修正仍为次级敏感性（自定义近似，非 Dutilleul 原法）。
  3) A14 全部改用 B=2999（s41e 曾误用 b=999，R5-P0-6），
     并新增 20-seed 蒙特卡洛稳定性列（s41g 追加）；
     A14 不再报"置换 p"（标签洗牌与区组设计无关）。
  4) 导出 data/block_membership_0p15.csv（R5-P0-7）。
  5) A7 仅保留 4 行干净 cap/nocap 对比。
输出: tables/_F1_v6.csv _F2_v6.csv TableA5_moran TableA14_block_sensitivity
      Table1/2/A1/A3/A4/A6/A7/A9 (v6) + data/stats_v6.json + data/block_membership_0p15.csv
"""
import json
import math
import os
...
BP = 9999      # label-shuffle permutation resamples (non-spatial baseline)
...
def label_perm_p(x, y, stat_fn, c=None, b=BP):
    """标准标签洗牌置换 p（non-spatial baseline，R5-P0-5 修复）:
    完全案例子集上随机重排 y，B=BP 次 Monte Carlo，双侧（向量化实现）。
    不保持任何空间结构——空间不确定性由区组 bootstrap CI 承担；
    本 p 仅作非空间参照基线，文中标注 non-spatial label permutation。"""
    x = np.asarray(x, float); y = np.asarray(y, float)
    cc = np.asarray(c, float) if c is not None else None
    ok = _ok_mask(x, y, c)
    xo, yo = x[ok], y[ok]
    if cc is None and stat_fn is spear_stat:
        n0 = len(yo)
        rx = stats.rankdata(xo); ry = stats.rankdata(yo)
        rxc = rx - rx.mean(); ryc = ry - ry.mean()
        r_obs = float((rxc @ ryc) / math.sqrt((rxc @ rxc) * (ryc @ ryc)))
        perms = np.argsort(RNGP.random((b, n0)), axis=1)
        rp = (ryc[perms] @ rxc) / math.sqrt((rxc @ rxc) * (ryc @ ryc))
        return float((np.sum(np.abs(rp) >= abs(r_obs) - 1e-12) + 1) / (b + 1))
    co = cc[ok] if cc is not None else None
    r_obs = stat_fn(xo, yo) if co is None else stat_fn(xo, yo, co)
    cnt = 1
    for _ in range(b):
        yp = RNGP.permutation(yo)
...
def fl_perm_p_partial(x, y, c, b=BP):
    """Freedman-Lane 秩残差置换 p（偏相关层，R5-P0-5 修复，向量化实现）:
    对秩 ry ~ rc 回归，置换其残差并加回拟合值构造 y*，
    再计算 x 秩残差与 y* 秩残差的相关。控制变量本身不动。"""
    x = np.asarray(x, float); y = np.asarray(y, float); c = np.asarray(c, float)
    ok = _ok_mask(x, y, c)
    xo, yo, co = x[ok], y[ok], c[ok]
    n0 = len(yo)
    rx, ry, rc = stats.rankdata(xo), stats.rankdata(yo), stats.rankdata(co)
    A = np.column_stack([rc, np.ones_like(rc)])
    P = A @ np.linalg.pinv(A)
    M = np.eye(n0) - P
    rx_ = M @ rx                       # 均值为 0（1 ∈ col(A)）
    fit_y = P @ ry
    resid_y = ry - fit_y
    ry_ = M @ ry
    den_obs = math.sqrt((rx_ @ rx_) * (ry_ @ ry_))
    r_obs = float((rx_ @ ry_) / den_obs) if den_obs > 0 else np.nan
    perms = np.argsort(RNGP.random((b, n0)), axis=1)
    R = resid_y[perms] @ M.T           # 每行 = M @ (置换后残差) = y* 的秩残差（均值0）
    num = R @ rx_
    den = math.sqrt(rx_ @ rx_) * np.sqrt((R ** 2).sum(axis=1))
    rp = num / np.where(den > 0, den, np.nan)
    cnt = int(np.sum(np.isfinite(rp) & (np.abs(rp) >= abs(r_obs) - 1e-12)))
...
for c, cn in MORPH:
    ...
    for t, tn in TARGETS:
        ...
        p_perm = label_perm_p(xc_full, yc_full, spear_stat)
...
for c, cn in MORPH:
    ...
    for t, tn in TARGETS:
        ...
        p_perm = fl_perm_p_partial(xc_full, yc_full, cc_full)
...
raw.to_csv("tables/_F1_v6.csv", index=False, encoding="utf-8-sig")
part.to_csv("tables/_F2_v6.csv", index=False, encoding="utf-8-sig")
...
# ---------- A14: 区组设计敏感性（尺寸 × 原点），统一 B=2999（R5-P0-6 修复） ----------
# 注: 标签洗牌置换 p 与区组设计无关，不再列入；20-seed MC 稳定性由 s41g 追加。
a14_rows = []
for c, t in HEAD:
    for size in [0.10, 0.15, 0.20]:
        for oi, origin in enumerate([(0.0, 0.0), (0.05, 0.05)]):
            bl = make_blocks(size, origin)
            _, lo, hi, _pb = block_boot_ci(df[c].values, df[t].values, spear_stat, b=B, blocks=bl)
...
# ---------- 敏感性：排除两个回退目标村（Zuyuan/Mulihong，覆盖关联专用） ----------
sens_fb = {}
sub3 = df[~df.village.isin(["Zuyuan 祖源", "Mulihong 木梨硔"])]
for c, cn in MORPH:
    for t in ["cov85_4p", "rsrp_p10_4p"]:
        ss = sub3[[c, t]].dropna()
        r, p = stats.spearmanr(ss[c], ss[t])
        sens_fb[f"{c}~{t}"] = dict(rho=round(r, 3), p=round(p, 4), n=len(ss))
...
# R5-P0-7: 区组成员关系同时导出为独立 CSV（不再只嵌在 JSON）
_bm = [{"block_id": k, "village": v_} for k, vs_ in _blocks_dbg.items() for v_ in vs_]
pd.DataFrame(_bm).to_csv("data/block_membership_0p15.csv", index=False, encoding="utf-8-sig")
